...
def train(epochs, G_A2B, G_B2A, D_A, D_B, optimizer_G_A2B, optimizer_G_B2A, optimizer_D_A, optimizer_D_B, dataloader_mri, dataloader_sos, old=True):
  ...
  for epoch in range(epochs):
    ...
    for i in range(1,8000): #enumerate(zip(dataloader_mri, dataloader_sos),0):
        # Set model input
...
if __name__ ==  '__main__':
    ...
    # G_A2B, G_B2A, D_A, D_B, optimizer_G_A2B, optimizer_G_B2A, optimizer_D_A, optimizer_D_B = load_models(name)
    G_A2B.to(device)
    G_B2A.to(device)
    D_A.to(device)
    D_B.to(device)
